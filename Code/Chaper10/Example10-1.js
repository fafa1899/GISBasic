...
import BuildingLayer from './lib/BuildingLayer.js'
...
(function () {
	...
	//建筑物要素
	const buildingLayer = new BuildingLayer(layerGroup, 'public/LosAngelesBuilding.gltf');
	...
	function CreateLight() {
		...
		buildingLayer.BuildingMaterial.uniforms.lightSourceDirection.value = directionalLight.position;
	...
	function AddSkyBox() {
		...
		loader.load([
			"public/clouds1/left.bmp",
			"public/clouds1/right.bmp",
			"public/clouds1/top.bmp",
			"public/clouds1/bottom.bmp",
			"public/clouds1/front.bmp",
			"public/clouds1/back.bmp",
		], function (texture) {
	...
	document.getElementById('toggleButton').addEventListener('click', function () {
		...
		document.getElementById('popupTable').classList.toggle('hidden');
		isPickup = !isPickup;
		if (!isPickup) {
			buildingLayer.BuildingMaterial.uniforms.pickupObjectId.value = parseFloat(0);
		}
	});
	...
	function Main() {
		...
		renderer.domElement.addEventListener('dblclick', (event) => {
			if (isPickup) {
				...
				if (intersects.length > 0) {
					...
					const ecefCoord = spatialCoordinateTransform.Enu2Ecef(new THREE.Vector3(intersectionPoint.x, -intersectionPoint.z, intersectionPoint.y));
					const wgs84Coord = spatialCoordinateTransform.Ecef2Gcs(new THREE.Vector3(ecefCoord.x, ecefCoord.y, ecefCoord.z));
					const utmCoord = spatialCoordinateTransform.Gcs2Utm(wgs84Coord);

					//
					const table = document.getElementById("property-table");
					table.rows[1].cells[1].innerHTML = wgs84Coord.x;
					table.rows[2].cells[1].innerHTML = wgs84Coord.y;
					table.rows[3].cells[1].innerHTML = wgs84Coord.z;

					let requestAddress = '/building/attribute?x=' + utmCoord.x + "&y=" + utmCoord.y;
						...
						.then(data => {
							if (data) {
								for (let ii = 0; ii < 11; ++ii) {
									table.rows[4 + ii].cells[1].innerHTML = data[table.rows[4 + ii].cells[0].innerHTML];
								}

								const objectId = data[table.rows[4].cells[0].innerHTML];
								buildingLayer.BuildingMaterial.uniforms.pickupObjectId.value = parseFloat(objectId);
							} else {
								for (let ii = 0; ii < 11; ++ii) {
									table.rows[4 + ii].cells[1].innerHTML = "";
								}
								buildingLayer.BuildingMaterial.uniforms.pickupObjectId.value = parseFloat(0);
							}
						})
		...
		controls.addEventListener('change', function () {
			...
			buildingLayer.BuildingMaterial.uniforms.cameraWorldPosition.value = camera.position;
