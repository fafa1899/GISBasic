...
void WriteDemFile(const TIN &demTin, const string &tinPath) {
  ofstream outfile(tinPath, ios::binary);
  if (!outfile) {
    printf("write file error %s\n", tinPath.c_str());
    return;
  }

  size_t vertexCount = demTin.finite_vertex_handles().size();
  size_t faceCount = demTin.finite_face_handles().size();
  ...
  vector<double> vertexTmp;
  map<TIN::Vertex_handle, int> vertexIndexMap;
  int vi = 0;
  for (const auto &vertexHandles : demTin.finite_vertex_handles()) {
    vertexTmp.push_back(vertexHandles->point().x());
    vertexTmp.push_back(vertexHandles->point().y());
    vertexTmp.push_back(vertexHandles->point().z());
    vertexIndexMap.emplace(vertexHandles, vi);
    vi++;
  }
  outfile.write((char *)vertexTmp.data(), vertexTmp.size() * sizeof(double));

  uint8_t triType = 3;
  size_t stepSize = 13;
  vector<char> indiceTmp(faceCount * stepSize);
  size_t fi = 0;
  for (const auto &f : demTin.finite_face_handles()) {
    size_t offset = fi * stepSize;
    memcpy(indiceTmp.data() + offset, &triType, sizeof(uint8_t));
    offset = offset + sizeof(uint8_t);

    for (int i = 0; i < 3; i++) {
      auto iter = vertexIndexMap.find(f->vertex(i));
      if (iter != vertexIndexMap.end()) {
        int id = iter->second;
        memcpy(indiceTmp.data() + offset, &id, sizeof(int));
  ...
  outfile.write(indiceTmp.data(), faceCount * stepSize);
}
...
int main(int argc, char *argv[]) {
  ...
  // Create DEM
  TIN demTin(points.points().begin(), points.points().end());

  //创建不规则三角网地形文件
  WriteDemFile(demTin, tinPath);
